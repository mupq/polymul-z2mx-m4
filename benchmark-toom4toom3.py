...
for i in range(1, 1025):
    if ceil(i / 4) & 1:  # skip toom4 for n with odd limb sizes
        continue
    if ceil(ceil(i / 4) / 3) & 1:  # skip toom4toom3 for odd limb sizes
        continue
    layers = 0
    while True:
        try:
            while True:
                t = ceil(ceil(i / 12) / (2 ** layers))
                if t < 6:
                    break
                if t > 48:
                    layers += 1
                    continue
                binary = f"benchmark-toom4toom3_{i}_{t}.bin"
